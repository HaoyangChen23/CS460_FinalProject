...
def find_corners(img: np.ndarray) -> np.ndarray:
    ...
    edges = cv2.adaptiveThreshold(magnitude, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 11, 2)
...
def _detect_lines(img: np.ndarray) -> np.ndarray:
    ...
    # convert line endpoints to Hough space representation
    hough_lines = []
    for line in lines:
        x1, y1, x2, y2 = line[0]
        # calculate line's rho and theta
    ...
    # filter lines close to horizontal and vertical
